...
// TestHolderEmptyDefault — F-24: 초기 상태는 빈 Config (= 실행 명령 없음 + 포트 기본값).
func TestHolderEmptyDefault(t *testing.T) {
	h := NewHolder()
	snap := h.Snapshot()
	if len(snap.RunCommands) != 0 {
		t.Fatalf("RunCommands=%v want []", snap.RunCommands)
	}
...
func TestHolderReplaceSnapshotRoundTrip(t *testing.T) {
	...
	want := protocol.AgentConfigData{
		RunCommands:   []string{"npm ci", "npm run build"},
		ContainerPort: 3000,
	}
	h.Replace(want)
	got := h.Snapshot()
	if len(got.RunCommands) != 2 || got.RunCommands[0] != "npm ci" || got.RunCommands[1] != "npm run build" {
		t.Fatalf("RunCommands=%v", got.RunCommands)
	}
...
func TestHolderDeepCopyOnReplace(t *testing.T) {
	h := NewHolder()
	external := []string{"a", "b"}
	h.Replace(protocol.AgentConfigData{RunCommands: external})

	// 외부 슬라이스 mutate.
	external[0] = "MUTATED"

	snap := h.Snapshot()
	if snap.RunCommands[0] != "a" {
		t.Fatalf("Holder leaked external slice: got %q want %q", snap.RunCommands[0], "a")
	}
}
...
func TestHolderDeepCopyOnSnapshot(t *testing.T) {
	h := NewHolder()
	h.Replace(protocol.AgentConfigData{RunCommands: []string{"a", "b"}})
	s1 := h.Snapshot()
	s1.RunCommands[0] = "MUTATED"
	s2 := h.Snapshot()
	if s2.RunCommands[0] != "a" {
		t.Fatalf("snapshot mutation leaked: got %q", s2.RunCommands[0])
	}
}
...
func TestHolderConcurrentReplaceSnapshot(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < N; i++ {
			h.Replace(protocol.AgentConfigData{
				RunCommands:   []string{"cmd-a", "cmd-b"},
				ContainerPort: i + 1,
			})
		}
	}()
	go func() {
		...
		for i := 0; i < N; i++ {
			...
			if len(snap.RunCommands) != 0 && len(snap.RunCommands) != 2 {
				t.Errorf("inconsistent snapshot len=%d", len(snap.RunCommands))
				return
			}
		}
	}()
