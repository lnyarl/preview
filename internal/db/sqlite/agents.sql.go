...
const resetAllOnlineAgents = `-- name: ResetAllOnlineAgents :execrows
UPDATE agents SET status = 'offline' WHERE status = 'online'
`

func (q *Queries) ResetAllOnlineAgents(ctx context.Context) (int64, error) {
	result, err := q.db.ExecContext(ctx, resetAllOnlineAgents)
	if err != nil {
		return 0, err
	}
